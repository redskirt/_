...
object WebDigg extends App {
  ...
  val captchaBase64Str = Util.getMatched(loginContent, captcha_regex)
  println("captchaStr --> " + captchaBase64Str)

  val str = """
    {"showapi_res_code":0,"showapi_res_error":"","showapi_res_body":{"Result":"haue","ret_code":0,"Id":"95f4d921-78c8-41bf-bc78-48a33ca9be56"}}
  """
//  println(getResultCaptchaCode(str))
  
  val response = post(url_post_account_login, 
      Map(("mobile" -> "sw"), ("code" -> "ss"), ("account" -> DEFAULT_ACCOUNT), ("password" -> DEFAULT_PASSWORD), 
          ("captcha" -> ""), ("is_30_days_no_login" -> "false"), ("service", "https://www.toutiao.com/")), 
      Map()
  )
   ...
   * 登陆，三方验证码解析
   */
  def doLogin(account: String, password: String, captchaStr: String): (Boolean, HttpResponse) = {
    var _requltCaptchaCodeJson = invokeParseCaptcha(captchaStr)
    var _resultCaptchaCode = getResultCaptchaCode(_requltCaptchaCodeJson)
    
    if (_resultCaptchaCode != "-1" /*验证码有返回*/ ) {
      // 尝试执行登陆，返回状态码判断
      post(url_post_account_login, Map(), Map())
      if(true /*登陆成功*/) {
    	  // Login
      } else {
        doLogin(account, password, Util.getMatched(refreshGET(url_get_login), captcha_regex))
      }
    } else {
      // 重调，传入新串
     doLogin(account, password, Util.getMatched(refreshGET(url_get_login), captcha_regex))
  ...
  def invokeParseCaptcha(captchaStr: String): String = {
    ...
    parseContent(responseCpatcha.getEntity.getContent)
  }
...
=======
   * 原生POST请求
>>>>>>> 6ed23d5477663c74161ae5f7e50455bc03bf7a2b
   */
